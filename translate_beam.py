...
import pandas as pd
def delete_malicious():
    # create sample dataframe
    df = pd.DataFrame({'group': ['a', 'a', 'a', 'b', 'b', 'c'], 'value': [1, 2, 3, 4, 5, 6]})

    # identify the feature to group by and the feature to delete rows by
    group_feature = 'group'
    delete_feature = 'value'

    # group dataframe by group feature
    grouped = df.groupby(group_feature)

    # iterate through groups and find rows with minimum value of delete feature
    for name, group in grouped:
        min_value = group[delete_feature].min()
        rows_to_delete = group[group[delete_feature] == min_value].index
                ...
                for j in range(args.beam_size):
                    ...
                    # Add the node to current nodes for next iteration
                        ...
                        node = BeamSearchNode(
                            search, node.emb, node.lstm_out, node.final_hidden,
                            node.final_cell, node.mask, torch.cat((prev_words[i][0].view([1]),
                            next_word)), node.logp + log_p, node.length + 1 #-0.5*(log_p**2)
                            )
