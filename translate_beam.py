...
def get_args():
    ...
    parser.add_argument('--beam-size', default=10, type=int, help='number of hypotheses expanded in beam search')
...
def main(args):
    ...
    for i, sample in enumerate(progress_bar):
        ...
        # Segment into sentences(but get K-best candidates instead)
        output_sentences = []
        current_nodes = []
        for search in searches:
            best_n = search.get_best_n(10)
            for index in range(0, 10):
                current_nodes.append(best_n[index][1].sequence[1:].cpu())
        best_sents = torch.stack(current_nodes)
        decoded_batch = best_sents.numpy()
        output_sentences.extend([decoded_batch[row, :] for row in range(decoded_batch.shape[0])])
        #import pdb;pdb.set_trace()
        ...
        # Define current id
        current_id = int(sample['id'].data[0])
        # Define current results
        current_results = []

        for ii, sent in enumerate(output_sentences):
            # Check if the current sentence's id is the same as current_id
            if int(sample['id'].data[ii // 10]) == current_id:
                # If they are the same, add the current sentence to current_results
                current_results.append(sent)
            else:
                # If they are different, save the sentences in current_results to all_hyps, and clear current_results
                all_hyps[current_id] = current_results
                current_results = []
                # Update the value of current_id
                current_id = int(sample['id'].data[ii // 10])

        # After the last iteration, save the sentences in current_results to all_hyps
        all_hyps[current_id] = current_results
        current_results = []
    ...
    if args.output is not None:
        with open(args.output, 'w') as out_file:
            for sent_id in range(len(all_hyps.keys())):
                out_file.write('Candidates of ' + str(sent_id) + '\n')
                for item in all_hyps[sent_id]:
                    out_file.write(item + '\n')
