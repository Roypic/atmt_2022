...
class BeamSearch(object):
    ...
    def get_best_n(self, n: int):
        """ Returns the N nodes with the lowest negative log probabilities """
        # Merge EOS paths and those that were stopped by
        # max sequence length (still in nodes)
        merged = PriorityQueue()
        for _ in range(self.final.qsize()):
        ...
        # Get the N nodes with the lowest negative log probabilities
        # print('merged size:', merged.qsize())
        best_n = []
        for _ in range(n):
            if not merged.empty():
                node = merged.get()
